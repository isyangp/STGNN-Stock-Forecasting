...
def plot_results_comparison(
    ...
    # 添加边界检查
    n_instances = min(n_random_instances, y_true.shape[0])
    n_nodes = min(n_random_nodes, y_true.shape[2])
    ...
    for i, l in enumerate(feature_names):
        ...
        # 使用边界检查后的随机实例和节点
        random_instances = sorted(
            np.random.choice(range(y_true.shape[0]), n_instances, replace=False))
        random_nodes = sorted(
            np.random.choice(range(y_true.shape[2]), n_nodes, replace=False))
...
def plot_results_fitness(
    ...
    for i, l in enumerate(feature_names):
        ...
        random_instances = sorted(
            np.random.choice(range(y_true.shape[0]), min(n_random_instances, y_true.shape[0]),
                             replace=False))
        random_nodes = sorted(
            np.random.choice(range(y_true.shape[2]), min(n_random_nodes, y_true.shape[2]),
                             replace=False))
